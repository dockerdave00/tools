#!/usr/bin/bash
     ...
     EC2_HOME="/home/ec2-user"
      ...
      HOST_IP=$(cat ${MY_HOME}/instance_ip.dat | awk '{print $1}')
    HTTP_PORT=$(cat ${MY_HOME}/instance_ip.dat | awk '{print $2}')
...
for FILE in app.py \
	...
	do
		if [ ${FILE} != "postgres_setup.sh" ]; then
			...
			if [ ${FILE} == "app.py" ]; then
				echo -e "\tpermission: chaning permissions to 755 on ${FILE}"
...
echo -e "\tpermission: Setup docker compose permissions"
ssh -i ${PEM_FILE} ec2-user@${HOST_IP} "sudo chmod +x /usr/local/bin/docker-compose"

echo -e "\tlink: Setup docker compose link"
RESULT=$(ssh -i ${PEM_FILE} ec2-user@${HOST_IP} "sudo ls -1 /usr/bin/docker-compose 2<&1")
